...
def entry(args: dict):
    ...
    if str(args['kernels']).__contains__(','):
        kernels = str(args['kernels']).split(',')
    else:
        kernels = [str(args['kernels'])]

    label = TrainingHelpers.get_label(args[S_METRIC])
    logger.info(f"Selected kernels:    {kernels}")
